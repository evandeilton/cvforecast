...
#' @export
cvForecastControl <- function (stepSize = 1, maxHorizon = 1, minObs = 2*max(cycle(x)), fixedWindow = TRUE, summaryFunc = tsSummary, preProcess = FALSE, ppMethod = "guerrero", cvMethod="MAPE", tsfrequency="month", OutlierClean = TRUE) {
    list(stepSize = stepSize, maxHorizon = maxHorizon, minObs = minObs,
		...
		tsfrequency=tsfrequency, OutlierClean=OutlierClean)
}

#' Default plot for Cross-validation forecast
#' @export
plot.cvforecast <- function(obj, cv_forecast = TRUE, ...) {
...
#' Choose best method based on trend and linearity
#' @export
...
#' Autofind time sieries number of differences
#' @export
...
#' Generate forecast horizon time sequency
#' @export
...
#' Switcher funtion to choose frenquencies based on char dates
#' @export
